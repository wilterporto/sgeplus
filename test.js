    ...
    function updateItemChart(items) {
        ...
        const data = items.map(i => i.correct_perc);

        const getBgColor = v => {
            if (v >= 70) return 'rgba(25, 135, 84, 0.7)'; // success
            if (v >= 50) return 'rgba(255, 193, 7, 0.7)'; // warning
            return 'rgba(220, 53, 69, 0.7)'; // danger
        };
        const getBorderColor = v => {
            if (v >= 70) return 'rgb(25, 135, 84)';
            if (v >= 50) return 'rgb(255, 193, 7)';
            return 'rgb(220, 53, 69)';
        };

        if (itemChart) {
            itemChart.data.labels = labels;
            itemChart.data.datasets[0].data = data;
            itemChart.data.datasets[0].backgroundColor = data.map(getBgColor);
            itemChart.data.datasets[0].borderColor = data.map(getBorderColor);
            // Update tooltip callback to include new items details
            itemChart.options.plugins.tooltip.callbacks.label = function(context) {
                const item = items[context.dataIndex];
                return [
                    ` Acertos: ${Number(item.correct_perc).toFixed(2)}% (${item.correct_count}/${item.total_count})`,
                    ` Erros: ${Number(item.incorrect_perc).toFixed(2)}% (${item.incorrect_count}/${item.total_count})`,
                    ` Sem Resposta: ${Number(item.blank_perc).toFixed(2)}% (${item.blank_count}/${item.total_count})`
                ];
            };
            itemChart.update();
        } else {
            ...
            itemChart = new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [{
                        label: '% de Acerto',
                        data: data,
                        backgroundColor: data.map(getBgColor),
