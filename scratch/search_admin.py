...
print(f"Buscando 'admin.py' nos logs em {root_dir}...")
        ...
        if file.endswith((".py", ".pb", ".jsonl", ".txt", ".md")):
            ...
            try:
                with open(full_path, "rb") as f:
                    content = f.read()
                if b"admin_bp" in content or b"def list_imports" in content or b"DietaryRestriction" in content:
...
print(f"\nEncontrados {len(found_files)} arquivos contendo referências a admin:")
    ...
    if path.endswith(".jsonl"):
        ...
        with open(path, "r", encoding="utf-8", errors="ignore") as f:
            for line_num, line in enumerate(f, 1):
                if "def list_imports" in line or "admin_bp = Blueprint" in line or "DietaryRestrictionForm" in line:
                    try:
                        obj = json.loads(line)
                        def check(val, key_path):
                            if isinstance(val, str) and ("admin_bp" in val or "list_imports" in val) and len(val) > 2000:
                                cand_path = f"c:\\Users\\pc\\source\\sgeplus\\scratch\\extracted_admin_jsonl_{line_num}_{len(val)}.py"
